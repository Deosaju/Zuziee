...
function index() {
  ...
  function getevents(){
    async function getUser() {
      try {
        const response = await axios.post(`http://localhost:5000/followers`, { accessToken });
        console.log("User Data", response.data);
      ...
      <div className="relative sm:-8 p-4 bg-[#13131a] min-h-screen flex flex-row">
        ...
        <div className="flex-1 max-sm:w-full max-w-[1280px] mx-auto sm:pr-5">
          ...
          <div className="bg-[#1f1f2e] rounded-md p-4 mt-4">
            <h1 className="text-white font-bold text-xl">Followers</h1>
            <button className="bg-[#2f2f3e] rounded-md p-4 mt-4 flex flex-row items-center" onClick={getevents}>Show followers</button>
